...
export const MaterialTest = memo(function MaterialTest() {
  ...
  const [platformMaterial, setPlatformMaterial] = useState<
    "default" | "mirror"
  >("default");
  const [currentMaterial, setCurrentMaterial] = useState<THREE.Material | null>(
    null,
  );
  const [currentMeshType, setCurrentMeshType] = useState<MeshType>(
    meshTypes[1],
  );
  ...
  useEffect(() => {
    ...
    if (sceneBackgroundsRef.current[sceneBackgroundName]) {
      ...
      if (sceneBackgroundName === "SpruitSunrise") {
        platformDefaultMaterial.color = new THREE.Color().setHSL(
          0.13,
          0.61,
          0.08,
        );
  ...
  useEffect(() => {
    (async () => {
      const [map, displacementMap, normalMap, roughnessMap, aoMap] =
        await api.createTexturesFromImages(
          [
          ...
          Object.keys(maps).forEach((prop) => {
            if (Object.hasOwn(materialsRef.current[type], prop)) {
              ...
              if (prop === "displacementMap") {
                (
                  materialsRef.current[type] as THREE.MeshStandardMaterial
                ).displacementScale = 1;
              } else if (prop === "aoMap") {
                (
                  materialsRef.current[type] as THREE.MeshStandardMaterial
      ...
      setCurrentMaterial(
        materialsRef.current[customPropsRef.current.currentMaterialType],
      );
    })();
  }, []);
  ...
  return (
    <>
      ...
      <mesh
        ...
        receiveShadow
        material={
          platformMaterial === "default"
            ? platformDefaultMaterial
            : mirrorMaterial
        }
